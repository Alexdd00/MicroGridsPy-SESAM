...
def Operation_Emission(params,risultato):
    ...
    for y in range(years):
        ...
        if Generator_Partial_Load==1:
            ...
            RES_emission = sum( RES_Unit_CO2_emission.iloc[r,0]* RES_Nominal_Capacity.iloc[r,0]/1e3 for r in range(params['RES_Sources']))
            GEN_emission = sum( Generator_Nominal_Capacity.iloc[g,0]/1e3* Generator_Unit_CO2_emission.iloc[g,0] for g in range(n_generators))
