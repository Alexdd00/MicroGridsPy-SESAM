...
def Operation_Cost(params,risultato):
    ...
    for y in range(years):
        ...
        OyM_Ren = sum(( RES_Nominal_Capacity.iloc[r,0]* RES_Specific_Investment_Cost.iloc[r,0]* RES_Specific_OM_Cost.iloc[r,0]) for r in range(params['RES_Sources']))    
        OyM_Gen = sum(( Generator_Nominal_Capacity.iloc[g,0]* Generator_Specific_Investment_Cost.iloc[g,0]* Generator_Specific_OM_Cost.iloc[g,0]) for g in range(n_generators))
        ...
        Battery_cost_in[y] = sum( Battery_Inflow.iloc[t,y]* Unitary_Battery_Replacement_Cost for t in range(periods))
        Battery_cost_out[y] = sum( Battery_Outflow.iloc[t,y]* Unitary_Battery_Replacement_Cost for t in range(periods))
        Battery_Replacement_Cost[y] = Battery_cost_out[y] + Battery_cost_in[y]

        #Calculation Lost Load cost
                
        Lost_Load_Cost[y] = sum( Lost_Load.iloc[t,y]* Lost_Load_Specific_Cost for t in range(periods))
        ...
        if Generator_Partial_Load==1:
            ...
            Fuel_Cost[y] = sum( Total_Fuel_Cost.iloc[y,g] for g in range(n_generators))
        ...
        if y==years_selected:
            Demand[y] = sum(Demand_df.iloc[t,y] for t in range(periods))
